...
def matrix_solve_tridiagonal(matrix_storage,vector):
    #matrix_storage is a n*3 matrix, where each row represents the 3 non-zero values in the tridiagonal n*n matrix
    tempDiagonal = [matrix_storage[i][1] for i in range(len(matrix_storage))]
    n = len(matrix)
    ...
    for i in range(1, n):# we start at n-2, because the very last entry in matrix_storage is irrelivant, and not techinuqally in the matrix n*n.
        w = matrix_storage[i][0]/tempDiagonal[i-1]
        tempDiagonal[i] -= w* matrix_storage[i-1][2]
    ...
    for i in range(n-2, -1, -1):
        solution[i] = (vector[i] - matrix_storage[i][2] *solution[i+1])/tempDiagonal[i]
    return solution
